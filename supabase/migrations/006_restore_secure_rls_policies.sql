-- Restore secure RLS policies for profiles table

-- First, drop the temporary policy
DROP POLICY IF EXISTS "Allow all operations temporarily" ON public.profiles;

-- Disable RLS temporarily to clean up
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;

-- Drop any remaining policies to start fresh
...
-- Allow service role to insert profiles (for registration flow)
CREATE POLICY "Allow service role profile creation" ON public.profiles
