-- =============================================
-- اعلان دیتابیسی برای درخواست دوستی (پشتیبان سمت سرور)
-- ⚠️ برای جلوگیری از اعلان تکراری در اپ، یکی از این دو را انتخاب کنید:
--   الف) این trigger را فعال نگه دارید و disable_friendship_notification_trigger.sql را اجرا نکنید
--      + در اپ dedupe فعال است.
--   ب) trigger را با disable_friendship_notification_trigger.sql غیرفعال کنید
--      + فقط کلاینت Flutter اعلان می‌سازد (توصیه‌شده).
-- =============================================
...
CREATE TRIGGER friendship_request_notify_trigger
  AFTER INSERT ON public.friendship_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_on_friendship_request_insert();
