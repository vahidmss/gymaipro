...
class ConfidentialUserInfoService {
  ...
  static Future<bool> saveConsentAccepted() async {
    try {
      ...
      // فعال کردن دستاورد ثبت اطلاعات محرمانه
      try {
        final context = MyApp.navigatorKey.currentContext;
        if (context != null) {
          // استفاده از Provider برای دسترسی به AchievementService
          // اگر Provider در دسترس نبود، از طریق main context استفاده می‌کنیم
          try {
            final achievementService = AchievementService.instance;
            await achievementService.updateProgress('confidential_info', 1);
            debugPrint('✅ Confidential info achievement unlocked');
          } catch (e) {
            debugPrint('⚠️ Error unlocking confidential info achievement: $e');
          }
        }
      } catch (e) {
        // بی‌صدا - اگر AchievementService در دسترس نبود
        debugPrint('⚠️ Could not unlock confidential info achievement: $e');
      }
