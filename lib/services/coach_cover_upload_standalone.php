<?php
/**
 * فایل مستقل آپلود تصویر کاور موزیک مربی
...
// 1. بررسی JWT Token: اول هدرها، در نهایت فیلد فرم auth_token (برای nginx/hostهایی که هدر را strip می‌کنند)
...
if (!in_array($user_role, ['admin', 'trainer'])) {
    ...
    echo json_encode([
        'success' => false,
        'error' => 'forbidden',
        'message' => 'Only admins and trainers can upload covers',
    ]);
    exit;
}
...
echo json_encode([
    'success' => true,
    'cover_url' => $cover_url,
    'image_url' => $cover_url, // برای سازگاری با کد Dart
    'url' => $cover_url, // برای سازگاری بیشتر
    'file_name' => $file_name,
    'file_size' => $file['size'],
    'trainer_id' => $user_id,
