...
class _TrainerProfileScreenState extends State<TrainerProfileScreen> {
  ...
  Future<void> _loadTrainerData() async {
    ...
    try {
      ...
      _trainerDetail =
          await _trainerService.getTrainerDetails(widget.trainerId);
      ...
      if (_reviews.isNotEmpty) {
        final totalRating =
            _reviews.fold(0, (sum, review) => sum + review.rating);
      ...
      if (_currentUserId.isNotEmpty) {
        final relationships =
            await _trainerService.getClientTrainers(_currentUserId);
        _relationship = relationships.firstWhere(
          (rel) => rel.trainerId == widget.trainerId,
          orElse: () => null as TrainerClient, // Will be null if not found
        );
  ...
  Future<void> _requestTrainer() async {
    try {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('خطا در ارسال درخواست')),
        );
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> _cancelRequest() async {
    try {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('خطا در لغو درخواست')),
        );
      }
    } catch (e) {
      setState(() {
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              fontSize: 16,
                              color:
                                  _avgRating > 0 ? Colors.black87 : Colors.grey,
                            ),
                          ),
                        ],
                      ),
      ...
      if (_relationship!.isPending || _relationship!.isActive) {
        return SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label:
                Text(_relationship!.isPending ? 'لغو درخواست' : 'قطع همکاری'),
