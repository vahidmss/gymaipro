...
class WorkoutLogDetailsScreen extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            _buildSummaryCard(
                totalSets, totalExercises, totalWeight, totalTime),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text('سوپرست - ${exercise.tag}',
                        style: const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold,
                            fontSize: 13)),
                  ),
                  Text('${exercise.exercises.length} تمرین',
                      style: const TextStyle(
                          color: AppTheme.goldColor, fontSize: 11)),
                ],
              ),
              ...
              const SizedBox(height: 8),
              ...exercise.exercises
                  .map((item) => _buildSupersetItemCard(item))
                  .toList(),
            ],
          ),
        ),
  ...
  Widget _buildSupersetItemCard(SupersetItemLog item) {
    final totalReps =
        item.sets.fold<int>(0, (sum, s) => sum + (s.reps ?? s.seconds ?? 0));
    final totalWeight =
        item.sets.fold<double>(0, (sum, s) => sum + (s.weight ?? 0));
        ...
        children: [
          Row(
            children: [
              ...
              Text('${item.sets.length} ست',
                  style:
                      const TextStyle(color: AppTheme.goldColor, fontSize: 10)),
            ],
          ),
          const SizedBox(height: 4),
          Row(
  ...
  String _buildShareSummary(WorkoutDailyLog log, String persianDate,
      int totalSets, int totalExercises, double totalWeight, int totalTime) {
